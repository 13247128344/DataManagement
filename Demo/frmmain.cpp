...
HWND frmMain::getHwnd()
{
	ID_kong = ui->widgetVideoArea->winId();
	HWND h = (HWND)ID_kong;
	return h;
}
...
void frmMain::initForm()
{
    ...
    ui->widgetVideoArea->setStyleSheet(qss.join(""));
...
{
    if (image.height()>0){
     //   QPixmap pix = QPixmap::fromImage(image.scaled(widgets[0]->width(),widgets[0]->height()));
...
//        }
        ...
        for(auto x : widgets)
        {
            pix = QPixmap::fromImage(image.scaled(x->width(),x->height()));
            x->setPixmap(pix);
        }
    }
}

const QPixmap* frmMain::getScreenshot()
{
    return widgets[0]->pixmap();
}
